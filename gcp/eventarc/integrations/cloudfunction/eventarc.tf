...
resource "google_eventarc_trigger" "finalized_object_trigger" {
    name = local.finalized_template_trigger
    location = var.REGION
...
resource "google_eventarc_trigger" "deleted_object_trigger" {
    name = local.deleted_template_trigger
    location = var.REGION
    labels = local.common_tags
