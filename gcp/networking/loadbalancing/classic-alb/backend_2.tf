...
resource "google_compute_health_check" "autohealing_2" {
...
resource "google_compute_instance_group_manager" "onebank_instance_group_2" {
    ...
    auto_healing_policies {
        health_check = google_compute_health_check.autohealing_2.id
        initial_delay_sec = 120
    }
}

resource "google_compute_autoscaler" "autoscaling_2" {
    name = "onebank-backend-autoscaler-2"
    target = google_compute_instance_group_manager.onebank_instance_group_2.id
    zone = data.google_compute_zones.private_2.names[0]
...
resource "google_compute_backend_service" "backend_2" {
    ...
    timeout_sec = 30
    health_checks = [ google_compute_health_check.autohealing_2.id ]

    backend {
        description = "Backend Group for managed instance group 2"
        group = google_compute_instance_group_manager.onebank_instance_group_2.instance_group
