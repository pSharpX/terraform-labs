...
resource "google_pubsub_subscription_iam_member" "email_subscriber_member" {
...
resource "google_pubsub_subscription_iam_member" "push_subscriber_member" {
  subscription = google_pubsub_subscription.push_notification_subscription.name
  role = "roles/pubsub.subscriber"
  member = "serviceAccount:${google_service_account.subscriber_sa.email}"
}

resource "google_pubsub_subscription_iam_member" "sms_subscriber_member" {
  subscription = google_pubsub_subscription.sms_notification_subscription.name
  role = "roles/pubsub.subscriber"
  member = "serviceAccount:${google_service_account.subscriber_sa.email}"
}

resource "google_pubsub_subscription_iam_member" "dlt_subscriber_member" {
  subscription = google_pubsub_subscription.dead_letter_subscription.name
  role = "roles/pubsub.subscriber"
  member = "serviceAccount:${google_service_account.subscriber_sa.email}"
}
